...
class TushareStockBasic(TushareBase, object):
    ...
    def delete_tushare_stock_basic(self):
        sql = "delete from " + self.table_name + " date='" + self.get_latest_work_day() + "'"
        self.delete_sql(sql)
    ...
    def insert_sunso_stock_basic(self):
        sql = "insert into " + self.t_sunso_stock_basic + "(code,name,industry,area,pe,pb,earnings_per_share," \
              "net_asset_per_share,circulation_stock_volume,totals_stock_volume,total_assets," \
              ...
              "select code,name,industry,area,pe,pb,esp,bvps,outstanding,totals,totalAssets,liquidAssets,fixedAssets," \
