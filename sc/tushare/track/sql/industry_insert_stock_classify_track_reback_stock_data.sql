...
select * from (
    ...
    {{reback_day}} as reback_day,cd.code,cd.name,
    round(avg(cd.close_pre_close_diff_amt_ratio),2)
    ...
    and cd.trade_date >=(
        select trade_date from (
            ...
            where trade_date < '{{track_date}}' order by trade_date desc limit {{reback_day}}
